...
import { formatCurrency, SAUCE_INFO, SIZE_INFO } from '../utils/mockData';
...
export default function SalesAnalytics({ orders }: SalesAnalyticsProps) {
  ...
  // Size popularity (generik, mendukung berapa pun jenis ukuran)
  ...
  const totalSizes = Object.values(sizeCounts).reduce((a, b) => a + b, 0);
  const sizeStats = Object.entries(sizeCounts).map(([sizeId, count]) => ({
    sizeId,
    name: SIZE_INFO[sizeId]?.name ?? sizeId,
    count,
    pct: totalSizes ? (count / totalSizes) * 100 : 0,
  }));
  const SIZE_BAR_COLORS = ['bg-red-500', 'bg-red-600', 'bg-amber-500', 'bg-orange-500', 'bg-rose-500'];
        ...
        <div className="bg-white border border-gray-200 p-5 rounded-2xl shadow-xs flex flex-col justify-between">
          ...
          <div>
            ...
            <div className="space-y-2 py-1">
              <div className="flex justify-between text-xs text-gray-500 font-bold flex-wrap gap-1">
                {sizeStats.map(s => (
                  <span key={s.sizeId}>{s.name} - {s.count} porsi</span>
                ))}
              </div>
              ...
              <div className="w-full h-5 bg-gray-100 rounded-full overflow-hidden flex p-0.5 border border-gray-200">
                {sizeStats.map((s, idx) => (
                  <div
                    key={s.sizeId}
                    style={{ width: `${s.pct}%` }}
                    className={`h-full ${SIZE_BAR_COLORS[idx % SIZE_BAR_COLORS.length]} ${idx === 0 ? 'rounded-l-full' : ''} ${idx === sizeStats.length - 1 ? 'rounded-r-full' : ''} flex items-center justify-center text-[10px] text-white font-black font-mono transition-all duration-1000`}
                  >
                    {s.pct > 15 ? `${s.pct.toFixed(0)}%` : ''}
                  </div>
                ))}
              </div>
              <div className="flex justify-between text-[10px] text-gray-400 font-bold pt-1 flex-wrap gap-1.5">
                {sizeStats.map((s, idx) => (
                  <span key={s.sizeId} className="flex items-center gap-1">
                    <span className={`w-2 h-2 rounded ${SIZE_BAR_COLORS[idx % SIZE_BAR_COLORS.length]}`} /> {s.name}
                  </span>
                ))}
              </div>
            </div>

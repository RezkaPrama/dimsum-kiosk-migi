...
import { SAUCE_INFO, SIZE_INFO, formatCurrency, generateQRISPayload, formatWhatsAppMessage, getWhatsAppURL } from '../utils/mockData';
...
export default function CustomerKiosk({ settings, onSubmitOrder, orders, activeCookingStatuses, availableSauces }: CustomerKioskProps) {
  ...
  const [extraSauces, setExtraSauces] = useState<Record<SauceType, number>>(() => Object.fromEntries(Object.keys(SAUCE_INFO).map(id => [id, 0])));
  ...
  // Auto-reset ke layar awal setelah pesanan sukses (bantu kiosk siap dipakai customer berikutnya)
  const [autoResetCountdown, setAutoResetCountdown] = useState<number>(18);

  // Auto-adjust piece-sauces list size when size changes
  const handleSizeChange = (size: DimsumSize) => {
    setSelectedSize(size);
    const pcs = SIZE_INFO[size]?.pcs ?? 3;
    setItemPcs(pcs);
  ...
  const resetCustomizer = () => {
    ...
    setItemPcs(pcs);
    setPieceSauces(Array(pcs).fill(firstAvail));
    setExtraSauces(Object.fromEntries(Object.keys(SAUCE_INFO).map(id => [id, 0])));
    setItemNotes('');
    setItemQuantity(1);
    setActiveCustomizingPiece(0);
  };
  ...
  // Auto-reset kiosk ke layar awal 18 detik setelah pesanan sukses, supaya
  // siap dipakai customer berikutnya tanpa perlu disentuh staff.
  useEffect(() => {
    if (screen !== 'success') {
      setAutoResetCountdown(18);
      return;
    }
    ...
    const timer = setTimeout(() => setAutoResetCountdown(prev => prev - 1), 1000);
    return () => clearTimeout(timer);
  }, [screen, autoResetCountdown]);

  // Handle adding custom item to cart
  const handleAddToCart = () => {
    const basePrice = SIZE_INFO[selectedSize]?.basePrice ?? 0;
  ...
  const handleAddPresetToCart = (size: DimsumSize, sauce: SauceType) => {
    const pcs = SIZE_INFO[size]?.pcs ?? 3;
    const basePrice = SIZE_INFO[size]?.basePrice ?? 0;
  ...
  return (
    <div id="customer_kiosk_device" className="w-full max-w-lg mx-auto min-h-screen bg-transparent text-gray-900 relative flex flex-col font-sans select-none">

      {/* Main Content Wrapper */}
      <div className="flex-1 pb-4 flex flex-col overflow-y-auto relative">
        ...
        {screen === 'home' && (
          <div className="flex flex-col flex-1 p-4 space-y-4">
            ...
            <div className="space-y-3.5">
              <h3 className="text-xs font-extrabold uppercase tracking-wider text-gray-700 flex items-center gap-1">
                <span className="w-1 h-3.5 bg-red-600 rounded-full inline-block"></span>
                Pilih Ukuran Dimsum
              </h3>
              
              {Object.values(SIZE_INFO).map((sizeInfo) => (
                <div key={sizeInfo.id} className="bg-white/80 backdrop-blur-md rounded-2xl overflow-hidden border border-white/60 hover:border-red-200 transition-all duration-300 flex flex-col relative group shadow-lg hover:shadow-2xl hover:-translate-y-0.5">
                  <div className="relative h-32 w-full bg-gray-100">
                    <img
                      src={sizeInfo.image}
                      alt={`Dimsum ${sizeInfo.name}`}
                  ...
                  <div className="p-3 flex flex-col justify-between flex-1">
                    <div>
                      <div className="flex justify-between items-start">
                        <h4 className="font-extrabold text-gray-900 text-sm">Dimsum {sizeInfo.name}</h4>
                        <span className="text-red-600 font-black font-mono text-sm">{formatCurrency(sizeInfo.basePrice)}</span>
                      </div>
                      {/* <p className="text-[10px] text-gray-500 mt-1 font-medium">{sizeInfo.description}</p> */}
                      <span className="text-red-600 font-black font-mono text-sm">{sizeInfo.description}</span>
        ...
        {screen === 'customize' && (
          <div className="flex flex-col flex-1 p-4 space-y-4">
            ...
            {/* Petunjuk cara memilih saus */}
            <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 flex gap-2 items-start">
              <Info className="w-4 h-4 text-blue-500 shrink-0 mt-0.5" />
              <p className="text-[10px] text-blue-700 leading-relaxed font-medium">
                <strong>Cara custom saus:</strong> 1) Tap salah satu dimsum di bawah untuk memilihnya, 2) Pilih jenis saus dari daftar, ulangi untuk tiap butir. Atau pakai tombol "Set Semua Butir" untuk saus yang sama semua.
              </p>
            </div>
            ...
            <div className="bg-white p-4 rounded-2xl border border-gray-200 text-center space-y-3 shadow-xs">
              <span className="inline-flex items-center gap-1 text-[10px] uppercase font-extrabold tracking-wider text-red-600 bg-red-50 border border-red-100 px-2.5 py-1 rounded-full">
                ...
                {SIZE_INFO[selectedSize]?.pcs ?? itemPcs} Butir Dimsum
              </span>
                ...
                {pieceSauces.map((sauce, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        style={{ backgroundColor: sInfo.color }}
                        className="absolute -top-2 left-1/2 -translate-x-1/2 border border-white text-[7px] text-white font-bold px-1.5 py-0.5 rounded-full whitespace-nowrap shadow-sm"
                      >
                        {sInfo.name.split(' ')[1] || 'Original'}
                      </span>
                    </button>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                {(Object.keys(SAUCE_INFO) as SauceType[])
                  ...
                  .map((sauceId) => {
                  const sInfo = SAUCE_INFO[sauceId];
                  const qty = extraSauces[sauceId] ?? 0;
                  const isAvailable = availableSauces[sauceId];
            ...
            <div className="mt-auto bg-white p-3 rounded-2xl border border-gray-200 flex items-center justify-between shrink-0 shadow-sm">
              ...
              >
                Checkout - {formatCurrency((SIZE_INFO[selectedSize]?.basePrice ?? 0) * itemQuantity + (Object.entries(extraSauces).reduce((s, [_, q]) => s + (q as number) * 3000, 0) * itemQuantity))}
              </button>
            </div>
                ...
                return (
                  <div key={item.id} className="bg-white border border-gray-200 p-3 rounded-xl flex justify-between gap-3 relative shadow-xs">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-extrabold text-gray-900">Dimsum {SIZE_INFO[item.size]?.name ?? item.size} ({item.pcs} pcs)</span>
          ...
          <div className="flex flex-col flex-1 p-4 space-y-4 text-center">
            ...
            <div className="bg-white p-5 rounded-3xl mx-auto border-4 border-gray-100 max-w-[240px] text-gray-900 shadow-xl flex flex-col items-center">
              ...
              <div className="w-40 h-40 bg-white rounded-xl p-1 flex items-center justify-center relative border border-gray-100 shadow-inner">
                <img 
                  src={settings.qrisImageUrl || `https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(generateQRISPayload(placedOrder.id, placedOrder.totalAmount))}`}
                  alt="QRIS Barcode"
          ...
          <div className="flex flex-col flex-1 p-4 space-y-4 text-center">
            ...
            <div className="mt-auto shrink-0 space-y-2">
              ...
              <p className="text-[9px] text-gray-400 font-medium">
                Kembali otomatis ke layar awal dalam {autoResetCountdown} detik...
              </p>
            </div>
